...
dbscan::kNNdistplot(Mlda@gamma, k =  5)
abline(h = 0.4, lty = 2)
# fpc package
res.fpc_lda <- fpc::dbscan(Mlda@gamma, eps = 0.008, MinPts = 5)
# dbscan package
db_lda <- dbscan::dbscan(Mlda@gamma, 0.008, 5)

all(res.fpc_lda$cluster == db_lda)
fviz_cluster(res.fpc_lda, Mlda@gamma, geom = "point")
print(db_lda)

#PCA to DB Scan:
#set.seed(123)
#db_pca <- fpc::dbscan(Mpca$x, eps = 15, MinPts = 5)
dbscan::kNNdistplot(Mpca$x, k =  5)
abline(h = 0.4, lty = 2)
# fpc package
res.fpc_pca <- fpc::dbscan(Mpca$x, eps = 15, MinPts = 5)
# dbscan package
db_pca <- dbscan::dbscan(Mpca$x, 15, 5)

all(res.fpc_pca$cluster == db_pca)
fviz_cluster(res.fpc_pca, Mpca$x, geom = "point")
print(db_pca)
